 ...
 */
void redirect(Program *prog, int cur, int n_prog) {
    if(cur == 0) {
        close(prog[cur].fd_in);
        dup2(prog[cur].fd_out, PIPEFD_WRITE);
        close(prog[cur].fd_out);
    }
    else if(cur == n_prog - 1) {
        dup2(prog[cur].fd_in, PIPEFD_READ);
        close(prog[cur].fd_in);
    }
    else {
        close(prog[cur + 1].fd_in);
        dup2(prog[cur].fd_in, PIPEFD_READ);
        close(prog[cur].fd_in);
...
void close_fds(Program *prog, int cur, int n_prog) {
    if(cur == 0) {
        close(prog[cur].fd_out);
    }
    else if (cur == n_prog - 1){
...
{
    if((programs[cur].pid = fork()) < 0) 
        die("fork() failed");

    if(programs[cur].pid == 0) {
        redirect(programs, cur, num_programs);

        execvp(programs[cur].argv[0], programs[cur].argv);
        die("execvp() failed");
    }

    close_fds(programs, cur, num_programs);
}
...
void prepare_pipes(Program *programs, int num_programs)
{
    int pipe_fd[2];
    if(pipe(pipe_fd) == -1)
        die("first pipe() failed");
    
    programs[0].fd_in = pipe_fd[PIPEFD_READ];
    programs[0].fd_out = pipe_fd[PIPEFD_WRITE];

    if(num_programs == 2) {
        programs[1].fd_in = pipe_fd[PIPEFD_READ];
        programs[1].fd_out = FD_STDOUT;
        return;
    }

    int i;
    for(i = 1; i < num_programs - 1; i++) {
        int pipe_fd[2];
        ...
        if(i == 1)
            programs[i].fd_in = programs[i - 1].fd_in;
        
        programs[i].fd_out = pipe_fd[1];
        programs[i + 1].fd_in = pipe_fd[0];
        programs[i + 1].fd_out = FD_STDOUT;
    }
}

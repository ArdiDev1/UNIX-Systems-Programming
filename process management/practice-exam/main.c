...
void parse_command_line(int argc, char *argv[], int *seed, int *n,
                        int *print_sorted, int *num_printed);
...
void parse_command_line(int argc, char *argv[], int *seed, int *n,
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-u")) {
      ...
      *num_printed = atoi(&argv[i][2]);
      if (*num_printed < 0) {
        fprintf(stderr,
                "number of integers to be printed must be a non-negative "
                "integer: %s\n",
                &argv[i][2]);
        exit(EXIT_FAILURE);
      }
      ...
      *n = atoi(argv[i]);
      if (*n <= 0 || *n > MAX_N || *n % 2) {
        fprintf(
            stderr,
            "The number of elements must be a postive even integer <= %d.\n",
            MAX_N);
        exit(EXIT_FAILURE);
      }
      flag = 1;
  ...
  if (!flag) {
    fprintf(stderr,
            ...
            "-p<N>   : number of integers to print. Default value is 0, which "
            "prints all.\n"

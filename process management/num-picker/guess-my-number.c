...
// check if number is right, increment attemps, if 
// it is, return zero, if not, return - 1;
...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    while(res != 0) {
        read(fdp[PIPEFD_READ], &guess, sizeof(int));
        
        res = gmn_check(&gmn, guess);

        write(fdc[PIPEFD_WRITE], &res, sizeof(int));
...
int main(int argc, char *argv[])
{
    ...
    do { 
        ...
        write(fdp[PIPEFD_WRITE], &guess, sizeof(int));

        read(fdc[PIPEFD_READ], &result, sizeof(int));

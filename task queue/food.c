...
typedef struct {
	int size;				  // max items in each col 
    int buf[MAX][3];		  // sliding tray   
    int remain;				  // remaining items (3*n_cons)
...
void add_to_buffer(int item, int col, two_d_buffer *p)
{
	...
	while (p->counts[col] >= p->size && p->remain != 0) {
		pthread_cond_wait(&p->produce_cond, &p->mutex);
	}

	prepare(item);
	
	int top = p->counts[col];
...
void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p)
{
	...
	//fill in code below
	pthread_mutex_lock(&p->mutex);
	while(!p->counts[0] || !p->counts[1] || !p->counts[2]) {
	...
	*c = p->buf[0][2];
	...
	for (size_t row = 1; row < p->size; row++) {
		for (size_t col = 0; col < 3; col++) {
			p->buf[row - 1][col] = p->buf[row][col];
		}
	}
...
{
	int id;				// id prim 
    list_t *p;          // shared linked list 
    two_d_buffer *q;    // shared  2d-buffer 
...
{   
	...
	while(!done)
	{
		...
		//fill in code below
		pthread_mutex_lock(&p->mutex);
		node* order = remove_first(&p->head, &p->tail);
		if(order) {
			add_to_buffer(order->v, order->v, q);
			free(order);
			my_data->total++;
		}
		else if(!order || !q->remain) {
			done = 1;
		}
...
int main(int argc, char *argv[])
{
	...
	p->head = NULL;
	p->tail = NULL;
	pthread_mutex_init(&p->mutex, NULL); // implement link list mutex
  
	//initilize the 2d buffer
	two_d_buffer *q = malloc(sizeof(two_d_buffer));
    q->size = size;										   // max num. items in each col.
    q->remain = 3*n_consumer;							   //remaining items  
    q->counts[0] = 0; q->counts[1] = 0; q->counts[2] = 0;  //init indices to 0
